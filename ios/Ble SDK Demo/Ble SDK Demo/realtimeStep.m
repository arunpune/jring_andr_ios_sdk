...
- (IBAction)startRealtimeStep:(UIButton *)sender {
    if([[NewBle sharedManager] isConnectOrConnecting]==YES){
        ...
        if(isStep==NO)
        {
            ...
            data= [[BleSDK sharedManager] RealTimeDataWithType:1];
        }
        else
        {
            isStep = NO;
            [sender setTitle:LocalForkey(@"开启实时计步") forState:UIControlStateNormal];
            data= [[BleSDK sharedManager] RealTimeDataWithType:0];
        }
...
- (IBAction)startSpo2:(UIButton *)sender {
    if([[NewBle sharedManager] isConnectOrConnecting]==YES){
        ...
        if(isStartSpo2==NO)
        {
            ...
            data= [[BleSDK sharedManager] manualMeasurementWithDataType:spo2Data measurementTime:30 open:YES];
        }
        else
        {
            isStartSpo2 = NO;
            [sender setTitle:LocalForkey(@"开启血氧测量") forState:UIControlStateNormal];
            data= [[BleSDK sharedManager] manualMeasurementWithDataType:spo2Data measurementTime:30 open:NO];
        }
        [[NewBle sharedManager] writeValue:SERVICE characteristicUUID:SEND_CHAR p:[NewBle sharedManager].activityPeripheral data:data];
    }
    else
...
- (IBAction)startHeartRate:(UIButton *)sender {
    if([[NewBle sharedManager] isConnectOrConnecting]==YES){
        ...
        if(isStartHeartRate==NO)
        {
            ...
            data= [[BleSDK sharedManager] manualMeasurementWithDataType:heartRateData measurementTime:30 open:YES];
        }
        else
        {
            isStartHeartRate = NO;
            [sender setTitle:LocalForkey(@"开启心率测量") forState:UIControlStateNormal];
            data= [[BleSDK sharedManager] manualMeasurementWithDataType:heartRateData measurementTime:30 open:NO];
        }
        [[NewBle sharedManager] writeValue:SERVICE characteristicUUID:SEND_CHAR p:[NewBle sharedManager].activityPeripheral data:data];
    }
    else
